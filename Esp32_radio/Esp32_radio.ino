...
//*                 By Ed Smallenburg. Mod by Andrea Console                                        *
//***************************************************************************************************
...
// 14-04-2020,~AC: New tft library, new screen content
// 15-04-2020,~AC: Longpress (no need to click) -> stop streaming; short click to resume
// 29-04-2020,~AC: Alarm function added, web interface changed and optimized for smartphone use
//
// Define the version number, also used for webserver as Last-Modified header and to
// check version for update.  The format must be exactly as specified by the HTTP standard!
#define VERSION     "Mon, 29 Apr 2020 18:45:00 GMT"
...
//**************************************************************************************************
void        displaytime ( const char* str, const char* alarmTime, bool alarmOn, uint16_t color = 0xFFFF ) ;  // ~AC added alarmOn
...
enum enc_menu_t { RADIO, MUSICFILE } ;                        // State for rotary encoder menu
enc_menu_t        enc_menu_mode = RADIO ;               // Default is RADIO mode
//~AC alarm clock stuff
char              alarmtimeinfo [6] ;                    // ~AC alarm time
char              alarmvolume [3] = "30" ;               // Initial volume of the alarm
sv bool           alarmon = false ;                      // Is the alarm active?
sv bool           alarmfired = false ;                   // Is the alarm ringing (playing)?
  ...
  while ( true )                                        // Find all mp3 files
  {
    ...
    else
    {
      ...
      if ( filename.endsWith ( ".mp3" ) )               // It is a file, but is it an MP3?
      {
        ...
        {
          SD_outbuf += tmpstr + String ("/") +          // Form line for mp3play_html page ~AC added: String("/") + as a separator
...
void IRAM_ATTR timer100()
{
  ...
  sv bool      oldstate ;                         // Previous state of input signal
  char         timeinfotxt[5] ;                   // Converted timeinfo
  ...
  if ( ( count10sec % 10 ) == 0 )                 // One second over?
  {
    ...
    scanios = 0 ;
    // ~AC longclick detection - read current state of SW pin
    newstate = ( digitalRead ( ini_block.enc_sw_pin ) == LOW ) ;
    if ( newstate && oldstate ) // && ( datamode != STOPPED ) ) ~AC
    {
      longclick = true ;
    }
    oldstate = newstate ;
    // ~AC end lonclick detection
    if ( ++timeinfo.tm_sec >= 60 )                // Yes, update number of seconds
    {
      ...
      {                                           
        if (!alarmfired)                          //~AC check alarm
        {
          sprintf ( timeinfotxt, "%02d:%02d",     // if !alarmfired, format current time to a string
                    timeinfo.tm_hour, 
                    timeinfo.tm_min ) ;     
        }
        else
        {
          analyzeCmd("upvolume", "6") ;           // if alarmfired, raise volume every minute
        }
        // ~AC check for alarm time 
        dbgprint("%s vs %s", timeinfotxt, alarmtimeinfo) ;                    
        if ((strcmp(timeinfotxt, alarmtimeinfo)==0) && alarmon && !alarmfired)
        {
          enc_menu_mode = RADIO ;                // since it fires doubleclick, it will switch to SD card
          doubleclick = true ;
          analyzeCmd("volume", alarmvolume) ;    // Set the initial volume                                 
          alarmfired = true ;
        }
      }
    }
...
void IRAM_ATTR isr_enc_switch()
{
  ...
  if ( newstate != sw_state )                              // State changed?
  {
    ...
    if ( !sw_state )                                       // SW released?
    {
      ...
      //{
      //  longclick = true ;                                 // Yes, register longclick ~AC longclick detection done in timer100()
      //}
      //else
      {
...
bool connecttohost()
{
  ...
  tftset ( 0, "" ) ;                     // Set screen segment text top line
  displaytime ( "" , "" , false) ;                  // Clear time on TFT screen ~AC added false
...
//**************************************************************************************************
// Return preset-, tone-, volume and alarm status.                                                        *
...
String getradiostatus()
{
  ...
  return String ( "preset=" ) +                          // Add preset setting
         ...
         String ( "\nalarmh=" ) +                        // Alarm hour
         String ( alarmtimeinfo ).substring(0,2) +
         String ( "\nalarmm=" ) +                        // Alarm minute
         String ( alarmtimeinfo ).substring(3,5) +
         String ( "\nalarmv=" ) +                        // Add initial volume setting 
         String ( alarmvolume ) +
         String ( "\nalarmon=" ) +                       // Add alarm status
         String ( alarmon ) ;
}
...
void setup()
{
  ...
  Serial.println() ;
  // ~AC Initialize Alarm time
  strncpy ( alarmtimeinfo, "00:00", sizeof(alarmtimeinfo)) ;
       ...
       ( ini_block.tft_scl_pin >= 0 ) )
  {
    ...
    if ( dsp_begin() )                                   // Init display
    {
      ...
      dsp_println ( tmpstr ) ;
      dsp_println ( "By Ed Smallenburg & Andrea Console" ) ;
  ...
  dsp_erase() ;                                             // Final display cleanup
}
...
void chk_enc()
{
  ...
  if ( tripleclick )                                          // First handle triple click (~AC to reset)
  {
    dbgprint ( "Triple click") ;
    tripleclick = false ;
    resetreq = true ;
    alarmfired = false ;                                      // mutes the alarm
  }
  ...
  if ( doubleclick )                                          // Handle the doubleclick
  {
    ...
    if (enc_menu_mode==RADIO)
    {
      ...
      { 
        ...
        enc_filename = getSDfilename ( "0" ) ;                // Set new filename
  ...
  if ( singleclick )
  {
    ...
    alarmfired = false ;                                      // mutes the alarm
    blset ( true ) ;
    if ( datamode == STOPPED )                                // ~AC once stopped, I wanted to start again with a short press
    {
      analyzeCmd("preset","0") ;
      muteflag = false ;                                      // Be sure muteing is off
      enc_menu_mode = RADIO ;
    }
    else if (enc_menu_mode==RADIO)                          // next radio station
    {
  ...
  if ( longclick )                                            // Check for long click
  {
    ...
    else
    {
      ...
      if ( alarmon )
      {
        tftset ( 3, "ALARM CLOCK ACTIVE" ) ; 
      }
      else
      {
        tftset ( 3, "ALARM CLOCK OFF" ) ; 
      }
      sleep ( 2 ) ;
      blset ( false ) ;
    }
  ...
  else if ( datamode == STOPPED )                            // Rotating when off is just for BL (time check - 2 secs)
  {
    sleep ( 2 ) ;
    blset ( false ) ;
  }
...
//                                        H A N D L E F S F                                        *
//**************************************************************************************************
// Handling of requesting pages from the PROGMEM. Example: favicon.png                             *
//**************************************************************************************************
void handleFSf ( const String& pagename )
{
  ...
  else
  {
    ...
    else if ( pagename.indexOf ( "favicon.png" ) >= 0 ) // Favicon icon is in PROGMEM
    {
      p = (char*)favicon_png ;
      l = sizeof ( favicon_png ) ;
    }
    else
    {
      p = index_html ;
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  else if ( argument.startsWith ( "alarm_" ) )     // alarm clock-related stuff  ~~~AC
  {
    if ( argument.indexOf ( "_time" ) > 0 )           // set alarm time
    {
      if (value.length() >= 5)
      {
        strncpy ( alarmtimeinfo, value.substring(0,5).c_str(), sizeof( alarmtimeinfo ) ) ;  
      }
      if (value.length() >= 8)                       // extract volume if included (http requests)
      {
        strncpy ( alarmvolume, value.substring(6,8).c_str(), sizeof( alarmvolume ) ) ;
      }
      if (value.length() >= 10)                      // extract status if included (http requests)
      {
        if (strncmp(value.substring(9,10).c_str(),"1",1) ==0)
        {
          alarmon = true ;
        }
        else
        ...
        nvssetstr ( "alarm_time", String ( value ) ) ; // Save current alarm settings
      }
    }          
    if ( argument.indexOf ( "_volume" ) > 0 )         // set alarm volume variable
    {
      strncpy ( alarmvolume, value.substring(0,2).c_str(), sizeof( alarmvolume ) ) ;             
    }
    if ( argument.indexOf ( "_status" ) > 0 )         // set alarm status (on, off)
    {
      if ( value == "OFF" )
      {
        alarmon = false ;
  ...
  else if ( argument.startsWith ( "backlight_on" ) )  // turn the backlight on
  {
    blset ( true ) ;
  }
  else
...
void displayinfo ( uint16_t inx )
{
  ...
  //}
  if (( tft )&&( inx > 0))                                               // TFT active? //never clear for the first line
  {
...
void handle_spec()
{
  ...
  if ( time_req )                                             // Time to refresh timetxt?
  {
    ...
    {
      displaytime ( timetxt , alarmtimeinfo , alarmon ) ;     //~AC
