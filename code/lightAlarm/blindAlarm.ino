//***************************************************************************************************
//*  lightAlarm -- Radio alarm project based on the ESP32_Radio project by Ed Smallenburg.          *
//*                By Andrea Console                                                                *
//*                All changes to the original code are all explicitly marked **AC**                *
//***************************************************************************************************
//***************************************************************************************************
...
// GPI012   -       -                   -                     -       Wired to the lighting LED **AC**
// -------  ------  ---------------     -------------------  ------   ----------------
...
//#define VERSION     "Fry, 16 Aug 2019 12:10:00 GMT"    //**AC**
// ESP32-Radio can be updated (OTA) to the latest version from a remote server.
// The download uses the following server and files:
//#define UPDATEHOST  "smallenburg.nl"                    // Host for software updates //**AC**
//#define BINFILE     "/Arduino/Esp32_radio.ino.bin"      // Binary file name for update software //**AC**
//#define TFTFILE     "/Arduino/ESP32-Radio.tft"          // Binary file name for update NEXTION image //**AC**
//
...
struct ini_struct
{
  ...
  int8_t         led_pin ;                            // GPIO connected to the LED //**AC**
...
#include "networkConfig_html.h" //**AC**
#include "index_html.h"
#include "musicConfig_html.h" //**AC**
#include "lightAlarm_css.h" //**AC**
...
int listsdtracks ( const char * dirname, int level = 0, bool send = true )
{
  ...
  if ( strcmp ( dirname, "/" ) == 0 )                   // Are we at the root directory?
  {
    ...
    if ( !SD_okay )                                     // See if known card
    {
      if ( send )
      {
        cmdclient.println ( "0/No tracks found" ) ;     // No SD card, empty list //**AC**
      }
      return 0 ;
    }
  }
...
void listNetworks()
{
  ...
  const char*      acceptable ;       // Network is acceptable for connection //**AC**
...
bool connecttohost()
{
  ...
  tftset ( 0, "BlindAlarm" ) ;                      // Set screen segment text top line //**AC**
...
void readIOprefs()
{
  ...
  struct iosetting klist[] = {                            // List of I/O related keys
    ...
    { "pin_led",       &ini_block.led_pin,          -1 }, // LED pin //**AC**
...
String readprefs ( bool output )
{
  ...
  if ( i == 0 )
  {
    outstr = String ( "No preferences found.\n"
                      "Use defaults or run BlindAlarm_init first.\n" ) ; //**AC**
  }
  return outstr ;
}
...
void setup()
{
  ...
  if ( about_html_version   < 170626 ) dbgprint ( wvn, "about" ) ; //**AC**todo
  if ( config_html_version  < 180806 ) dbgprint ( wvn, "config" ) ; //**AC**todo
  if ( index_html_version   < 180102 ) dbgprint ( wvn, "index" ) ; //**AC**todo
  if ( mp3play_html_version < 180918 ) dbgprint ( wvn, "mp3play" ) ; //**AC**todo
  if ( defaultprefs_version < 180816 ) dbgprint ( wvn, "defaultprefs" ) ; //**AC**todo
  // Print some memory and sketch info
  dbgprint ( "Starting BlindAlarm running on CPU %d at %d MHz.  Version %s.  Free memory %d",
             xPortGetCoreID(),
             ESP.getCpuFreqMHz(),
             VERSION,
...
void handleFSf ( const String& pagename )
{
  ...
  else
  {
    ...
    else if ( pagename.indexOf ( "lightAlarm.css" ) >= 0 )   // CSS file is in PROGMEM //**AC**
    {
      p = lightAlarm_css + 1 ; //**AC**
      l = sizeof ( lightAlarm_css ) ; //**AC**
    }
    else if ( pagename.indexOf ( "networkConfig.html" ) >= 0 ) // networkConfig page is in PROGMEM //**AC**
    {
      p = networkConfig_html ; //**AC**
      l = sizeof ( networkConfig_html ) ; //**AC**
    }
    else if ( pagename.indexOf ( "musicConfig.html" ) >= 0 ) // musicConfig page is in PROGMEM //**AC**
    {
      p = musicConfig_html ; //**AC**
      l = sizeof ( musicConfig_html ) ; //**AC**
    }
