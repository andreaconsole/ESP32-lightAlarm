...
//**************************************************************************************************
void        displaytime ( const char* str, uint16_t color = 0x7E0 ) ;
...
enum musicsource_t {RADIO, FILES} ;
musicsource_t     musicsource = RADIO ;
//
...
void tftset ( uint16_t inx, const char *str )
{
  if ( inx < TFTSECS )                                  // Segment available on display
  {
    ...
    {
      tftdata[inx].str = String ( str ).substring(0, 28) ;  // Yes, set string ~AC substring
    }
...
void tftset ( uint16_t inx, String& str )
{
  if ( inx < TFTSECS )                                  // Segment available on display
  {
    tftdata[inx].str = str.substring(0, 28) ;           // Set string ~AC substring
    tftdata[inx].update_req = true ;                    // and request flag
  }
}
...
bool connecttohost()
{
  ...
  tftset ( 0, "RADIO" ) ;                           // Set screen segment text top line
...
bool connectwifi()
{
  ...
  tftlog ( pfs ) ;                                      // Show IP
  delay ( 8000 ) ;                                      // Allow user to read this
  tftlog ( "\f" ) ;                                     // Select new page if NEXTION 
  dsp_erase() ;                                         // Clear screen
  dsp_setTextSize ( 2 ) ;                               // ~AC enlarge character font 
...
void setup()
{
       ...
       ( ini_block.tft_scl_pin >= 0 ) )
  {
    ...
    if ( dsp_begin() )                                   // Init display
    {
      ...
      dsp_println ( tmpstr ) ;
      dsp_println ( "By Ed Smallenburg & A. Console" ) ;
  ...
  if ( NetworkFound )                                    // OTA and MQTT only if Wifi network found
  {
    ...
    //~AC ArduinoOTA.setHostname ( NAME ) ;                    // Set the hostname
    //~AC ArduinoOTA.onStart ( otastart ) ;
    //~AC ArduinoOTA.begin() ;                                 // Allow update over the air
...
void chk_enc()
{
  ...
  if ( tripleclick )                                          // First handle triple click
  {
    ...
    tripleclick = false ;
    //~AC following lines: double and triple click functions exchanged
...
//         muteflag = !muteflag ;                                // Mute/unmute
        //~AC following lines until break: change station instead of mute
        if ( musicsource == RADIO )
        {
          dbgprint ( "go to next radio" ) ;
          enc_preset = ini_block.newpreset + 1 ;                    // set current preset + 1
          currentpreset = -1 ;                                  // Make sure current is different
          ini_block.newpreset = enc_preset ;                              // Request STOP, do not touch logclick flag
          singleclick = false ;
        } 
        else 
        {
          ...
          {
            singleclick = false ;                                    // Reset condition
            dbgprint ( "play random file" ) ;
            if ( get_FS_nodecount() )                               // Tracks on FS?
            {
  ...
  {
    longclick = false ;
    if (musicsource == FILES)
    {
      //play last radio
      dbgprint ( "radio mode" ) ;
      currentpreset = -1 ;
      ini_block.newpreset = enc_preset ;                   
      singleclick = false ;
      musicsource = RADIO ;
    }
    else
...
void loop()
{
  ...
  //~AC ArduinoOTA.handle() ;                             // Check for OTA
